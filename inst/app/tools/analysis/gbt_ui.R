...
output$ui_gbt_evar <- renderUI({
  if (not_available(input$gbt_rvar)) return()
  vars <- varnames()
  if (length(vars) > 0) {
...
output$ui_gbt_plots <- renderUI({
  ...
  selectInput(
    "gbt_plots", "Plots:", choices = gbt_plots,
...
output$ui_gbt <- renderUI({
  req(input$dataset)
  tagList(
    conditionalPanel(condition = "input.tabs_gbt == 'Summary'",
      wellPanel(
    ...
    wellPanel(
      conditionalPanel(
        ...
        radioButtons(
          "gbt_type", label = NULL, c("classification", "regression"),
        ...
        with(tags, table(
          tr(
            td(numericInput(
              "gbt_max_depth", label = "Max depth:", min = 1, max = 20,
              value = state_init("gbt_max_depth", 6)
            ), width = "50%"),
            td(numericInput(
              "gbt_learning_rate", label = "Learning rate:", min = 0, max = 1, step = 0.1,
        ...
        with(tags, table(
          tr(
            td(numericInput(
              "gbt_subsample", label = "Sub-sample:", min = 0.1, max = 1,
              value = state_init("gbt_subsample", 1)
            ), width = "50%"),
            td(numericInput(
              "gbt_nrounds", label = "# rounds:",
        ...
        with(tags, table(
          tr(
            td(numericInput(
              "gbt_early_stopping_rounds", label = "Early stopping:", min = 1, max = 10,
      ...
      conditionalPanel(
        ...
        selectInput(
          "gbt_predict", label = "Prediction input type:", reg_predict,
    ...
    help_and_report(
      ...
      fun_name = "gbt",
      help_file = inclMD(file.path(getOption("radiant.path.sklearn"), "app/tools/help/gbt.md"))
    )
  )
})
...
gbt_plot_width <- function()
  gbt_plot() %>% {if (is.list(.)) .$plot_width else 650}

gbt_plot_height <- function()
  gbt_plot() %>% {if (is.list(.)) .$plot_height else 500}
