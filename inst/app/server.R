...
shinyServer(function(input, output, session) {
  ...
  ## help ui
  output$help_sklearn_ui <- renderUI({
    sidebarLayout(
      sidebarPanel(
        help_data_panel,
        help_sklearn_panel,
      ...
      mainPanel(
        ...
        htmlOutput("help_sklearn")
      )
    )
  })

  ## packages to use for example data
  options(radiant.example.data = c("radiant.data", "radiant.sklearn"))
  ...
  if (!isTRUE(getOption("radiant.from.package"))) {
    ...
    cat("\nGetting radiant.sklearn from source ...\n")
  }

  ## source analysis tools for sklearn menu
